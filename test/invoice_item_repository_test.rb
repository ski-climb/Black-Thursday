...
class InvoiceItemRepositoryTest < Minitest::Test
  ...
  def test_it_returns_an_empty_array_when_no_invoice_items_have_given_item_id
    assert_equal [], @invoice_item_repository.find_by_item_id(1_000_000)
  end

  def test_it_returns_an_empty_array_when_no_invoice_items_have_given_invoice_id
    assert_equal [], @invoice_item_repository.find_by_invoice_id(1_000_000)
  end
  ...
  def test_invoice_items_can_all_be_found_by_item_id
    item_id = 234
    results = @invoice_item_repository.find_by_item_id(item_id)
  ...
  def test_invoice_items_can_all_be_found_by_invoice_id
    invoice_id = 23
    results = @invoice_item_repository.find_by_invoice_id(invoice_id)
    assert_equal 2, results.count
